"""Integration tests for MongoPublishingJobRepository using TestContainers."""
...
async def mongo_db(
    ...
    db = client["test_publishing_jobs"]
    yield db
    await client.drop_database("test_publishing_jobs")
    client.close()
...
) -> repository.MongoPublishingJobRepository:
    return repository.MongoPublishingJobRepository(mongo_db)


def _make_job(**overrides: object) -> models.PublishingJob:
    ...
    return models.PublishingJob(**defaults)  # type: ignore[arg-type]


class TestCreateJob:
    async def test_creates_job_with_provided_id(
        self, repo: repository.MongoPublishingJobRepository
    ) -> None:
        job = _make_job()
...
class TestStoreResult:
    async def test_stores_result_and_marks_completed(
        ...
        await repo.store_result(job.id, {"status": "completed", "summary": "All good"})

        result = await repo.get_job(job.id)
        assert result is not None
        assert result.status == models.JobStatus.COMPLETED
        assert result.result == {"status": "completed", "summary": "All good"}
