...
from app.critique.jobs import documents, models, service
...
def _make_service(
    ...
    submitted_jobs: list[models.ReviewJob] | None = None,
    ...
    captured: list[models.ReviewJob] = (
        submitted_jobs if submitted_jobs is not None else []
    )
    job_submitter = AsyncMock()

    async def _capture_submit(job: models.ReviewJob) -> None:
        captured.append(job)
...
class TestStartReview:
    ...
    async def test_returns_pending_job_on_success(self) -> None:
        ...
        job = await svc.start_review(uuid.uuid4())
