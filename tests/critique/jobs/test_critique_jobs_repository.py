"""Integration tests for MongoCritiqueJobRepository using TestContainers."""
...
async def mongo_db(
    ...
    db = client["test_critique_jobs"]
    yield db
    await client.drop_database("test_critique_jobs")
    client.close()
...
) -> repository.MongoCritiqueJobRepository:
    return repository.MongoCritiqueJobRepository(mongo_db)


def _make_job(**overrides: object) -> models.CritiqueJob:
    ...
    return models.CritiqueJob(**defaults)  # type: ignore[arg-type]


class TestCreateJob:
    async def test_creates_job_with_provided_id(
        self, repo: repository.MongoCritiqueJobRepository
    ) -> None:
        job = _make_job()
...
class TestStoreResult:
    async def test_stores_result_and_marks_completed(
        ...
        await repo.store_result(job.id, {"status": "review_completed", "iterations": 1})

        result = await repo.get_job(job.id)
        assert result is not None
        assert result.status == models.JobStatus.COMPLETED
        assert result.result == {"status": "review_completed", "iterations": 1}
