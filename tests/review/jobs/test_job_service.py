...
from app.review.jobs import documents, models, service
...
def _make_service(
    ...
    submitted_jobs: list[models.ReviewTask] | None = None,
    ...
    captured: list[models.ReviewTask] = (
        submitted_jobs if submitted_jobs is not None else []
    )
    job_submitter = AsyncMock()

    async def _capture_submit(job: models.ReviewTask) -> None:
        captured.append(job)
...
class TestStartReview:
    ...
    async def test_submits_job_with_document_text(self) -> None:
        doc_id = uuid.uuid4()
        submitted: list[models.ReviewTask] = []
        svc = _make_service(
