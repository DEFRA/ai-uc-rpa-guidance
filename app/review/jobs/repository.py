"""Repository for guidance review jobs."""
...
class AbstractReviewJobRepository(ABC):
    """Repository interface for review job persistence."""

    @abstractmethod
    async def create_job(self, job: models.ReviewJob) -> models.ReviewJob:
    ...
    async def store_result(self, job_id: uuid.UUID, result: dict[str, Any]) -> None:
        ...
        Args:
            job_id: The job UUID.
            result: The serialised ReviewResponse dict.
        """
...
class MongoReviewJobRepository(AbstractReviewJobRepository):
    """MongoDB-backed implementation of AbstractReviewJobRepository."""
    ...
    async def create_job(self, job: models.ReviewJob) -> models.ReviewJob:
        ...
        logger.info("Created review job %s", job.id)
        return job
    ...
    async def get_job(self, job_id: uuid.UUID) -> models.ReviewJob | None:
        ...
        if not doc:
            return None
        return models.ReviewJob.from_mongo_doc(doc)
    ...
    async def update_status(self, job_id: uuid.UUID, status: models.JobStatus) -> None:
        ...
        logger.info("Updated review job %s status to %s", job_id, status.value)
    ...
    async def store_result(self, job_id: uuid.UUID, result: dict[str, Any]) -> None:
        ...
        logger.info("Stored result for review job %s", job_id)
    ...
    async def set_error(self, job_id: uuid.UUID, message: str) -> None:
        ...
        logger.info("Set error for review job %s", job_id)
