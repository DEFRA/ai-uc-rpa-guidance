"""The ink colours a guidance document writes in, and the Markdown that carries them.

Markdown has no syntax for colour, so a coloured run is written as a Pandoc-style
bracketed span - `[text]{.red}`. The rule and the syntax live in one file, so the two
cannot drift apart.

The form matters as much as the fact of it. An inline `<span style>` would be worse
twice over: a style attribute is dropped by the editor's serialiser on the first save,
and it would not paint anything anyway under a `style-src 'self'` policy. HTML is also
not a container for Markdown - a span's contents are handed to an HTML parse, so
emphasis inside one comes back as literal asterisks. A bracketed span is content, and
its contents are re-tokenised as Markdown, so a mark inside a coloured run survives.
...
def name_for(value: str | None) -> str:
    ...
    with - and it is why the default colour needs no naming: Word spells the default
    out explicitly rather than leaving it unsaid, on most of the coloured runs in a
...
def marked_up(text: str, name: str) -> str:
    ...
    a bracket the author typed has to be escaped here, or it would close the span early
