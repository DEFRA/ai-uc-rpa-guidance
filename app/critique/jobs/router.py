"""FastAPI router for content review (critique) job endpoints."""
...
from app.critique.jobs import api_schemas, dependencies, service

logger = logging.getLogger(__name__)

router = fastapi.APIRouter(tags=["critique"])
...
async def submit_review(
    ...
    job_service: Annotated[
        service.CritiqueJobService,
        fastapi.Depends(dependencies.get_critique_job_service),
    ],
) -> api_schemas.CritiqueJobResponse:
    """Submit a guidance document for async content review.

    Accepts a document ID, creates a job record, and enqueues the review.
    Returns 202 immediately; poll GET /critique/jobs/{jobId} for results.

    Args:
        request: The review request containing the document ID.
        job_service: The critique job service, injected via FastAPI DI.
    ...
    return api_schemas.CritiqueJobResponse.from_job(job)
...
async def get_job(
    ...
    """Retrieve a content review job by ID.
...
@router.get(
    ...
    responses={
        fastapi.status.HTTP_404_NOT_FOUND: {
            "description": "No review run found for this document",
        },
    },
)
async def get_latest_review(
    ...
    """Retrieve the latest content review job for a guidance document.
