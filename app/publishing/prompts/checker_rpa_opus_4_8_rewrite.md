...
S2. Heading levels and numbering progress consistently (no ## → #### skips).
...
DEFAULT DIRECTION
For S1–S3, S5–S6: if in doubt, do NOT raise it; avoid false positives. The only
...
Instructions telling an END USER how to complete a template ("delete text in
red", "format to black before saving", "replace with case details", "complete
these fields") are valid content — do NOT flag. Flag only notes addressed to
the document's AUTHOR ("add detail here", "rewrite this"), or text whose
intended use is genuinely unclear. Flag incomplete sentences only if visibly
cut off (missing brackets, unfinished words, broken grammar).
...
5. Verdict: "Ready to send to Publishing" OR "Not ready – changes needed".
   Rule: any finding at medium/high/critical severity ⇒ Not ready; only info/low
   ⇒ Ready. Decide this LAST, after the findings, and keep it consistent with
   them.
